# Supabase Migration実行ガイド

## 実行手順

1. **Supabase SQL Editorを開く**
   - https://supabase.com/dashboard/project/onfqhnhdfbovgcnksatu/sql/new
...
1. **テーブル作成確認**
   - Table Editorで`users`と`user_profiles`テーブルが作成されていることを確認
...
エラーが発生した場合:
1. 既存のテーブルがある場合は、一度削除してから再実行
2. RLSポリシーが重複している場合は、既存のポリシーを削除