...
import { MapPin, List, Filter, ChevronDown, Heart, Map as MapIcon, Star, ArrowLeft, Navigation, X, CheckCircle } from 'lucide-react'
...
import { getUserGymCheckinStatus, type GymCheckinStatus } from '@/lib/supabase/checkin'
...
function SearchResultsContent() {
  ...
  const [checkinStatus, setCheckinStatus] = useState<Map<string, GymCheckinStatus>>(new Map())
  const sortControlRef = useRef<HTMLDivElement | null>(null)

  const { user, isAuthenticated } = useAuth()

  // Load check-in status for gyms
  const loadCheckinStatus = async (gymIds: string[]) => {
    if (!user || gymIds.length === 0) return

    try {
      const statusList = await getUserGymCheckinStatus(user.id, gymIds)
      const statusMap = new Map<string, GymCheckinStatus>()
      statusList.forEach(status => {
        statusMap.set(status.gym_id, status)
      })
  ...
  const fetchGyms = useCallback(async (conditions: {
    ...
    try {
      ...
      if (data) {
        ...
        // Load check-in status for authenticated users
        if (isAuthenticated && user) {
          loadCheckinStatus(uniqueGyms.map(gym => gym.id))
        }
      }
    } catch (err) {
                        ...
                        <div className="flex justify-between items-start gap-4">
                          <div>
                            ...
                            <div className="flex items-center gap-3 mt-2">
                              ...
                              {(() => {
                                const status = checkinStatus.get(gym.id)
